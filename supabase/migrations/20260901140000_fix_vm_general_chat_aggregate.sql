-- Upgrade vm_general_chat_aggregate to support optional dates (NULL), LEFT JOIN for calls,
-- richer sample records (summary, negative points, opportunities), and broad text search.
...
AS $$
  ...
  call_rows AS (
    SELECT
      ...
      COALESCE(NULLIF(af.metadata->>'agent_name', ''), NULLIF(af.metadata->>'user_name', ''), NULLIF(af.metadata->>'agent', ''), 'Desconocido')::text AS asesor,
      ...
      CASE
        WHEN public.vm_norm_text(COALESCE(a.overall_sentiment, af.sentiment)) LIKE '%positiv%' THEN 'positivo'
        WHEN public.vm_norm_text(COALESCE(a.overall_sentiment, af.sentiment)) LIKE '%negativ%' THEN 'negativo'
        ELSE 'neutral'
      END AS sentimiento,
      ...
      COALESCE(a.summary, af.summary, '')::text AS resumen,
      COALESCE(a.results->>'negative', a.results->>'puntos_negativos', '')::text AS puntos_negativos,
      COALESCE(a.results->>'opportunities', a.results->>'oportunidades', '')::text AS oportunidades,
      COALESCE(a.results->>'conclusions', a.results->>'conclusiones', '')::text AS conclusiones
  ...
  wa_rows AS (
    SELECT
      ...
      COALESCE(war.results->>'summary', war.results->>'resumen_ejecutivo', '')::text AS resumen,
      COALESCE(war.results->>'puntos_negativos', war.results->>'negative', '')::text AS puntos_negativos,
      COALESCE(war.results->>'oportunidades', war.results->>'opportunities', '')::text AS oportunidades,
      COALESCE(war.results->>'conclusiones', war.results->>'conclusions', '')::text AS conclusiones
    ...
    WHERE p_channel IN ('all', 'whatsapp')
      ...
      AND (p_start_date IS NULL OR COALESCE(wc.start_date, wc.created_at) >= p_start_date)
      AND (p_end_date IS NULL OR COALESCE(wc.start_date, wc.created_at) <= p_end_date)
  ...
  filtered AS (
    ...
    WHERE (p.sentiment_filter = '' OR public.vm_norm_text(u.sentimiento) LIKE '%' || p.sentiment_filter || '%')
      ...
      AND (
        p.motivo_filter = ''
        OR public.vm_norm_text(u.motivo_principal) LIKE '%' || p.motivo_filter || '%'
        OR public.vm_norm_text(u.promesa_pago) LIKE '%' || p.motivo_filter || '%'
        OR public.vm_norm_text(u.resumen) LIKE '%' || p.motivo_filter || '%'
        OR public.vm_norm_text(u.puntos_negativos) LIKE '%' || p.motivo_filter || '%'
        OR public.vm_norm_text(u.oportunidades) LIKE '%' || p.motivo_filter || '%'
        OR public.vm_norm_text(u.conclusiones) LIKE '%' || p.motivo_filter || '%'
      )
  ),
  summary AS (
    SELECT
  ...
  samples AS (
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'resumen', left(resumen, 500),
      'puntos_negativos', left(puntos_negativos, 400),
      'oportunidades', left(oportunidades, 400),
      'conclusiones', left(conclusiones, 400)
    ) ORDER BY fecha DESC), '[]'::jsonb) AS records
    FROM (SELECT * FROM filtered ORDER BY fecha DESC LIMIT 50) s
  )
...
GRANT EXECUTE ON FUNCTION public.vm_general_chat_aggregate(uuid, timestamptz, timestamptz, text, text, text, text, integer) TO authenticated;
