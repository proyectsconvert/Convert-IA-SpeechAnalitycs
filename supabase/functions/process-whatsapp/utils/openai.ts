...
/**
 * Sanitiza el contenido de un mensaje para que sea JSON-safe.
 * Elimina caracteres de control que rompen JSON.stringify.
 */
...
export async function createChatCompletion({
  ...
  // Sanitizar todos los mensajes antes de enviar
  ...
  // Validar que el JSON sea parseable antes de enviar
  let bodyStr: string;
  try {
    bodyStr = JSON.stringify(payload);
    JSON.parse(bodyStr); // doble check
  } catch (e) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // Si es rate limit (429) o server error (5xx), reintentar
      ...
      lastError = error;
      // Si es abort (timeout) o error de red, reintentar
